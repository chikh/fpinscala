...
object MyProgram:
  ...
  def fib(n: Int): Int =
    @annotation.tailrec
    def loop(currentN: Int, desiredN: Int, currentFib: Int, nextFib: Int): Int =
      if (currentN >= desiredN) then currentFib
      else loop(currentN + 1, desiredN, nextFib, currentFib + nextFib)
